...
class Sampler:
    ...
    def sample(
        self, percentage: float, eval_percentage=0.2
        ...
        """
        if percentage > 16.0 or percentage <= 0:
            raise Exception("invalid percentage. must be between 0 and 16.")

        sample_size = int((TRAIN_SIZE * percentage) / 100)
        size_eval = int(sample_size * eval_percentage)

        train_idx = self.generator.choice(TRAIN_SIZE - 1, sample_size + size_eval, replace=False)
        test_idx = self.generator.choice(TEST_SIZE - 1, sample_size + size_eval, replace=False)

        eval_train_idx = train_idx[-size_eval:]
        eval_test_idx = test_idx[-size_eval:]

        train_idx = train_idx[:-size_eval]
        test_idx = test_idx[:-size_eval]
